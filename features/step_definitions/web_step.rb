...
То(/^на странице не должно быть ссылки "(.*?)"$/) do |link|
  expect(page).to have_no_selector(:xpath, "//a[text()='#{link}']")
end
